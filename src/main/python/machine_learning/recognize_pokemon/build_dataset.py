...
pokemon_names_ls = ['Charmander', 'Bulbasaur', 'Squirtle']
for name in pokemon_names_ls:
    ...
    downloader.download(search_key, limit=1000, output_dir=train_dir,
                        adult_filter_off=False, force_replace=False, timeout=60)

    # Move five into the test folder
    file_names = os.listdir(os.path.join(train_dir, search_key))[:200]
